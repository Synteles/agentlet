...
output:
  ...
  show_turn_boundaries: false  # default is false
...
Agentlet-core supports passing images alongside the prompt to vision-capable models.

**CLI:**
```bash
agentlet-core --agentlet my-agent \
  --prompt "What does this architecture diagram show?" \
  --image /path/to/diagram.png \
  --image https://example.com/screenshot.png
```

**`--image` / `-i`** accepts any combination of:
- Local file paths (`.jpg`, `.jpeg`, `.png`, `.gif`, `.webp`)
- `http://` or `https://` URLs
- `data:image/...;base64,...` data URIs

Multiple `--image` flags are all passed to the model in the same request. Requires a vision-capable model (e.g., `claude-sonnet-4-6`, `gpt-4o`).
...
`.env` files loaded from (in order, first match wins):
...
See [Multi-Agent Guide](multi-agent.md) for a complete walkthrough.

## Next Steps

- **[Configuration Guide](configuration.md)** - Full config reference
- **[Multi-Agent Systems](multi-agent.md)** - Orchestrating sub-agentlets
